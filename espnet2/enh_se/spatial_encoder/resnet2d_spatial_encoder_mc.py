...
class SpatialResNetBranch(nn.Module):
    """
    マルチチャンネル専用のSpatial Embeddingモデル
    - MC枝: マルチチャンネル入力 -> 埋め込み
    """
    def __init__(
        self,
    ...
    def forward(self, X_mc, num_channels=None):
        """
        マルチチャンネル専用のforward
        Args:
            X_mc: [B, T, C_mc, F] - STFT spectrum (complex, C_mc=num_channels_mc)
            num_channels: Number of channels. Should be num_channels_mc.
        ...
        if X_mc.ndim != 4:
            raise ValueError(
                f"Expected 4D input tensor [B, T, C, F], but got {X_mc.ndim}D tensor with shape {X_mc.shape}"
            )
        ...
        if num_channels is not None:
            if num_channels != self.num_channels_mc:
                raise ValueError(
                    f"Expected num_channels={self.num_channels_mc}, but got {num_channels}"
                )
            if C != num_channels:
                raise ValueError(
                    f"Input shape indicates {C} channels, but num_channels={num_channels} was specified. "
                    f"Expected {self.num_channels_mc} channels."
                )
        else:
            if C != self.num_channels_mc:
                raise ValueError(
                    f"Expected {self.num_channels_mc} channels (multi-channel), but got {C} channels. "
                    f"Input shape: {X_mc.shape}"
                )
...
class ResNet2DSpatialEncoder(AbsSpatialEncoder):
    """Spatial ResNet Branch Encoder (Multi-channel only)"""
    ...
    def forward(
        ...
        Args:
            input: [B, T, C, F] - STFT spectrum (complex, multi-channel)
            ilens: [B] - input lengths (not used in this implementation)
            num_channels: Number of input channels. Should be num_channels_mc.
                         If None, automatically determined from input shape.
        Returns:
