...
class SpatialResNetBranch(nn.Module):
    ...
    def __init__(
        ...
        # 前処理モジュール（別々）
        self.ri_stack_sc = RIStack()
        self.ri_stack_mc = RIStack()
        ...
        # Trunk（別々）
        self.trunk_sc = ResNet18Trunk(in_channels=projection_dim)
        self.trunk_mc = ResNet18Trunk(in_channels=projection_dim)
        
        # Pooling + Embedding Head（別々）
        self.pooling_sc = GlobalStatsPool()
        self.pooling_mc = GlobalStatsPool()
        # Trunk出力は128チャネル、GlobalStatsPool後は256次元
        self.embedding_head_sc = EmbeddingHead(in_dim=256, embed_dim=embed_dim, eps=eps_embed)
        self.embedding_head_mc = EmbeddingHead(in_dim=256, embed_dim=embed_dim, eps=eps_embed)
    ...
    def forward_sc(self, X_sc):
        ...
        # RIスタック
        X_ri = self.ri_stack_sc(X)  # [B, 2, F, T] (real)
        
        # チャネル投影
        z = self.proj_sc(X_ri)  # [B, D, F, T] (D=projection_dim)
        
        # Trunk
        h = self.trunk_sc(z)  # [B, 128, F', T']
        
        # Pooling + Embedding
        g = self.pooling_sc(h)  # [B, 256]
        e = self.embedding_head_sc(g)  # [B, embed_dim]
    ...
    def forward_mc(self, X_mc):
        ...
        # RIスタック
        X_ri = self.ri_stack_mc(X)  # [B, 2*C_mc, F, T] (real)
        
        # チャネル投影
        z = self.proj_mc(X_ri)  # [B, D, F, T] (D=projection_dim)
        
        # Trunk
        h = self.trunk_mc(z)  # [B, 128, F', T']
        
        # Pooling + Embedding
        g = self.pooling_mc(h)  # [B, 256]
        e = self.embedding_head_mc(g)  # [B, embed_dim]
...
class ResNet2DDivSpatialEncoder(AbsSpatialEncoder):
    """Spatial ResNet Branch Encoder (div version with separate parameters for SC and MC)"""
