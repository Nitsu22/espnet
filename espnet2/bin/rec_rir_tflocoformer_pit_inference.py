...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Two-source TF-Locoformer CTF PIT inference"
    )
...
def main(cmd=None):
    ...
    model, _ = RIRTFLocoformerCTFTask.build_model_from_file(
    ...
    with SoundScpWriter(
        ...
        output_dir / "rir2", output_dir / "rir2" / "wav.scp"
    ) as writer2:
        for uid in uids:
            ...
            if rir.ndim != 2 or rir.shape[0] != 2:
                raise RuntimeError(
                    f"{uid}: expected estimated RIR shape [2, T], got {rir.shape}"
                )
