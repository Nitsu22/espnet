...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DARAS inference")
...
def main(cmd=None):
    ...
    model = model.to(dtype=dtype, device=args.device).eval()

    output_dir = Path(args.output_dir)
    reader = SoundScpReader(args.wav_scp, dtype=np.float32, always_2d=False)
    with SoundScpWriter(output_dir, output_dir / "wav.scp") as writer:
        for uid in reader:
            ...
            if wav.ndim == 2:
                wav = wav[:, 0]
            speech = torch.as_tensor(wav, dtype=dtype, device=args.device)
            with torch.no_grad():
