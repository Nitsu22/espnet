...
from espnet2.tasks.enh_rir import EnhancementRIRTask
from espnet2.torch_utils.device_funcs import to_device


_base.EnhancementTask = EnhancementRIRTask
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if rir_ref_lengths is None:
            rir_ref_lengths = rir_ref.new_full(
                [batch_size], dtype=torch.long, fill_value=rir_ref.size(1)
            )
        elif isinstance(rir_ref_lengths, np.ndarray):
            rir_ref_lengths = torch.as_tensor(rir_ref_lengths)
        rir_ref_lengths = rir_ref_lengths.to(dtype=torch.long)

        speech_mix = to_device(speech_mix, device=self.device)
        lengths = to_device(lengths, device=self.device)
        rir_ref = to_device(rir_ref, device=self.device)
        rir_ref_lengths = to_device(rir_ref_lengths, device=self.device)
        ...
        if self.segmenting and lengths[0] > self.segment_size * fs:
            ...
            for i in range_(num_segments):
                ...
                processed_wav = self._forward_enhance(
                    speech_seg, lengths_seg, rir_ref, rir_ref_lengths
                )
            ...
            waves = self._forward_enhance(speech_mix, lengths, rir_ref, rir_ref_lengths)
