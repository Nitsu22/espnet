...
class ESPnetEnhancementModel(AbsESPnetModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "speech_mix_mc" not in kwargs:
            raise ValueError("speech_mix_mc is required but not found in input kwargs.")
        speech_mix_mc = kwargs["speech_mix_mc"]
        ...
        if self.normalize_variance_per_ch:
            ...
            mix_std_mc = torch.std(speech_mix_mc, dim=dim, keepdim=True)
            speech_mix_mc = speech_mix_mc / mix_std_mc  # RMS normalization
            ...
            mix_std_ = torch.std(speech_mix, dim=dim, keepdim=True)
            speech_mix = speech_mix / mix_std_  # RMS normalization
            if speech_mix_mc.ndim > 2:
                dim_mc = (1, 2)
            else:
                dim_mc = 1
            mix_std_mc = torch.std(speech_mix_mc, dim=dim_mc, keepdim=True)
            speech_mix_mc = speech_mix_mc / mix_std_mc  # RMS normalization

        # model forward
        speech_pre, feature_mix, feature_pre, others = self.forward_enhance(
            speech_mix, speech_mix_mc, speech_lengths, additional, fs=fs
        )
    ...
    def forward_enhance(
        self,
        speech_mix: torch.Tensor,
        speech_mix_mc: torch.Tensor,
        speech_lengths: torch.Tensor,
        additional: Optional[Dict] = None,
        fs: Optional[int] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        feature_mix, flens = self.encoder(speech_mix, speech_lengths, fs=fs)
        feature_mix_mc, flens_mc = self.encoder(speech_mix_mc, speech_lengths, fs=fs)
        
        if additional is None:
            additional = {}
        additional["feature_mix_mc"] = feature_mix_mc
