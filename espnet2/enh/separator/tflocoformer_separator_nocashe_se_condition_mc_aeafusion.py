...
class TFLocoformerSeparatorSECondition(AbsSeparator):
    ...
    def __init__(
        ...
        spatial_encoder_path: Optional[str] = None,
        spatial_encoder_trainable: bool = False,
        ...
        if use_spatial_encoder:
            # Spatial encoderをseparatorの一部として構築
            self.spatial_encoder = ResNet2DDivSpatialEncoder(
            ...
            self.spatial_encoder_trainable = False
            self.aea_blocks = None
    ...
    def forward(
        ...
        # Calculate spatial embedding once
        spatial_emb = None
        if self.use_spatial_encoder and self.spatial_encoder is not None:
            if additional is None or "feature_mix_mc" not in additional:
                raise ValueError(
                    "feature_mix_mc is required in additional when "
        ...
        for ii in range(self.n_layers):
            if self.use_spatial_encoder and self.aea_blocks is not None:
                batch = self.aea_blocks[ii](batch, spatial_emb)
