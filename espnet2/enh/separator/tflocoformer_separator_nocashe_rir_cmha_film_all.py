...
class RIRCMHAFiLMBlock(nn.Module):
    """Frequency-wise RIR cross-attention followed by local FiLM."""
    ...
    def __init__(self, channels: int, n_heads: int = 4, dropout: float = 0.0):
        ...
        self.gamma_proj = nn.Conv2d(channels, channels, kernel_size=1)
        self.beta_proj = nn.Conv2d(channels, channels, kernel_size=1)
    ...
    def forward(self, speech: torch.Tensor, rir: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            rir: [B, C, T2, F]
        """
        bsz, ch, t1, n_freqs = speech.shape
        if rir.ndim != 4:
            raise ValueError(f"rir must be [B, C, T2, F], but got {rir.shape}")
        if rir.shape[0] != bsz or rir.shape[1] != ch or rir.shape[3] != n_freqs:
            raise ValueError(
                "RIR feature must match speech batch/channel/frequency: "
                f"speech={tuple(speech.shape)}, rir={tuple(rir.shape)}"
            )

        query = speech.permute(0, 3, 2, 1).reshape(bsz * n_freqs, t1, ch)
        key_value = rir.permute(0, 3, 2, 1).reshape(
            bsz * n_freqs, rir.shape[2], ch
        )
        context, _ = self.mha(query, key_value, key_value, need_weights=False)
        context = context.reshape(bsz, n_freqs, t1, ch).permute(0, 3, 2, 1)
        context = context.contiguous()
        gamma = self.gamma_proj(context)
        beta = self.beta_proj(context)
        return speech * gamma + beta
...
class TFLocoformerSeparator(AbsSeparator):
    ...
    def __init__(
        ...
        self.rir_conv = nn.Sequential(
            nn.Conv2d(num_spk * 2, emb_dim, ks, padding=padding),
            nn.GroupNorm(1, emb_dim, eps=eps),
        )
        self.rir_films = nn.ModuleList(
            [
                RIRCMHAFiLMBlock(
                    channels=emb_dim,
                    n_heads=n_heads,
    ...
    def forward(
        ...
        Args:
            ...
            additional (Dict or None): must include "rir_feat" [B, T2, 2, F].
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            rir_batch = self.rir_conv(rir_feat)  # [B, -1, T2, F]

        # separation
        for ii in range(self.n_layers):
            with torch.cuda.amp.autocast(enabled=False):
                batch = self.rir_films[ii](batch, rir_batch)  # [B, -1, T, F]
    ...
    def _get_rir_feat(
        ...
        if additional is None or "rir_feat" not in additional:
            raise ValueError("rir_feat is required in additional for RIR-CMHA.")

        rir = additional["rir_feat"]
        if rir.ndim != 4:
            raise ValueError(
                "rir_feat must be complex STFT [B, T2, 2, F], "
        ...
        rir = rir.transpose(1, 2)  # [B, 2, T2, F]
        return torch.cat((rir.real, rir.imag), dim=1)  # [B, 4, T2, F]
