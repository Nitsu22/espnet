...
class RIRCMHAFusionBlock(nn.Module):
    """Frequency-wise cross-attention from speech features to RIR features."""
    ...
    def forward(self, speech: torch.Tensor, rir: torch.Tensor) -> torch.Tensor:
        """Fuse RIR context into speech features.
        ...
        return self.fusion_net(torch.cat([speech, context.contiguous()], dim=1))
...
class TFLocoformerSeparator(AbsSeparator):
    ...
    def __init__(
        ...
        self.rir_fusions = nn.ModuleList(
            [
                RIRCMHAFusionBlock(
                    channels=emb_dim,
                    n_heads=n_heads,
    ...
    def forward(
        ...
        for ii in range(self.n_layers):
            with torch.cuda.amp.autocast(enabled=False):
                batch = self.rir_fusions[ii](batch, rir_batch)  # [B, -1, T, F]
