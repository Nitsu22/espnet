...
class AEAFusionBlock(nn.Module):
    """Adaptive Embedding Aggregation (AEA) + FiLM fusion block with cross-attention."""
...
class TFLocoformerSeparator(AbsSeparator):
    ...
    def __init__(
        ...
        # Apply AEA+FiLM conditioning once right after self.conv.
        self.aea_block = AEAFusionBlock(
            channels=emb_dim,
    ...
    def forward(
        ...
        if self.use_spatial_encoder:
            if additional is None or "spatial_embedding" not in additional:
                raise ValueError(
                    "spatial_embedding is required in additional for AEA+FiLM fusion."
                )
        ...
        # Apply spatial conditioning once before separator blocks.
        if self.use_spatial_encoder:
            batch = self.aea_block(batch, spatial_emb)  # [B, emb_dim, T, F]
