...
class TFLocoformerSeparatorMC(AbsSeparator):
    ...
    def __init__(
        ...
        self.n_imics = n_imics  # 追加: マイク数を保存

        t_ksize = 3
        ks, padding = (t_ksize, 3), (t_ksize // 2, 1)
        # 変更: Conv2Dの入力チャンネル数を 2 * n_imics に変更（early-fusion型マルチチャンネル対応）
    ...
    def forward(
        ...
        Args:
            input (torch.Tensor): batched audio tensor in TF-domain
                - [B, T, F] for monaural input (backward compatible)
                - [B, T, M, F] for multi-channel input
        ...
        """
        if input.ndim == 3:
            # モノラル入力の場合: [B, T, F] → [B, 1, T, F] に変換（後方互換性のため）
            batch0 = input.unsqueeze(1)  # [B, 1, T, F]
        elif input.ndim == 4:
            # マルチチャンネル入力の場合: [B, T, M, F] → [B, M, T, F] に transpose
            batch0 = input.transpose(1, 2)  # [B, M, T, F]
        else:
            raise ValueError(f"Unsupported input dimension: {input.ndim}. Expected 3 or 4.")

        # real/imagをチャンネル次元で結合: [B, M, T, F] → [B, 2*M, T, F]
        # early-fusion型マルチチャンネル対応（TF-GridNetV2と同様）
        ...
        # 各話者について単一チャンネルの複素STFTを出力
        # batch0を参照して複素テンソルの型を決定し、実数部・虚数部から複素テンソルを作成
        # batch[:, :, 0] と batch[:, :, 1] は [B, num_spk, T, F] の形状
        batch = new_complex_like(batch0, (batch[:, :, 0], batch[:, :, 1]))  # [B, num_spk, T, F]
        batch = [batch[:, src] for src in range(self.num_spk)]  # 各話者の [B, T, F] のリスト
