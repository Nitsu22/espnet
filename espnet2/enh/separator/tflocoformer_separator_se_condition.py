...
class TFLocoformerSeparatorSECondition(AbsSeparator):
    ...
    def __init__(
        ...
        if use_spatial_encoder:
            ...
            self.spatial_encoder = ResNet2DSpatialEncoder(
    ...
    def forward(
        ...
        if self.use_spatial_encoder and self.spatial_encoder is not None:
            # inputはfeature_mix: [B, T, F] (complex) なので、そのまま使用
            # SC入力として扱うため、num_channels=1を指定
            # trainableがFalseの場合は勾配計算を無効化
            if not self.spatial_encoder_trainable:
                with torch.no_grad():
                    spatial_emb = self.spatial_encoder(input, ilens, num_channels=1)  # [B, embed_dim]
            else:
                spatial_emb = self.spatial_encoder(input, ilens, num_channels=1)  # [B, embed_dim]
