...
class AEAFusionBlock(nn.Module):
    """FiLM fusion block without cross-attention."""

    def __init__(self, channels: int, emb_dim: int, n_heads: int = 4):
        super().__init__()
        _ = n_heads  # Keep signature compatible with existing configs.
    ...
    def forward(self, x: torch.Tensor, fixed_emb: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            fixed_emb: [B, emb_dim]
        """
        adaptive_emb = self.emb_proj(fixed_emb)  # [B, C]
        gamma = self.gamma_proj(adaptive_emb).unsqueeze(-1).unsqueeze(-1)
        beta = self.beta_proj(adaptive_emb).unsqueeze(-1).unsqueeze(-1)
        return x * gamma + beta
...
class TFLocoformerSeparator(AbsSeparator):
    ...
    def __init__(
        ...
        # Apply FiLM conditioning before every separator block
        # with layer-wise independent fusion modules.
        ...
        if film_init_identity:
            for aea_block in self.aea_blocks:
                aea_block.init_film_identity()
    ...
    def forward(
        ...
        Args:
            ...
            additional (Dict or None): must include "spatial_embedding" [B, D].
        ...
        if self.use_spatial_encoder:
            if additional is None or "spatial_embedding" not in additional:
                raise ValueError(
                    "spatial_embedding is required in additional for FiLM fusion."
                )
