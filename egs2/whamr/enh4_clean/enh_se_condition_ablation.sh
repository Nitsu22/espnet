...
eval_dsets_mode=all # all | test_only
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if $is_tse_task; then
            ...
            train_module=espnet2.bin.enh_train_se_condition_div
        fi
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        if [ "${eval_dsets_mode}" = "test_only" ]; then
            _eval_dsets="${test_sets}"
        else
            _eval_dsets="${valid_set} ${test_sets}"
        fi

        for dset in ${_eval_dsets}; do
            ...
            if $is_tse_task; then
                ...
                infer_module=espnet2.bin.enh_inference_se_condition_div_ablation
                _ablation_opts="--spatial_ablation_mode ${spatial_ablation_mode} "
                _ablation_opts+="--ablation_npz_data_dir ${ablation_npz_root}/${dset} "
                _ablation_opts+="--ablation_seed ${ablation_seed} "
                _ablation_opts+="--ablation_epoch ${ablation_epoch} "
                _ablation_opts+="--ablation_mean_source_mode ${ablation_mean_source_mode} "
                if [ -n "${ablation_pool_npz_scp}" ]; then
                    _ablation_opts+="--ablation_pool_npz_scp ${ablation_pool_npz_scp} "
                fi
                if [ -n "${ablation_mean_embedding_path}" ]; then
                    _ablation_opts+="--ablation_mean_embedding_path ${ablation_mean_embedding_path} "
                fi
            fi
            # shellcheck disable=SC2046,SC2086
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/enh_inference.JOB.log \
                ${python} -m ${infer_module} \
                    ...
                    ${_ablation_opts} ${_opts} ${inference_args} || { cat $(grep -l -i error "${_logdir}"/enh_inference.*.log) ; exit 1; }
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for score_obs in true false; do
            ...
            if [ "${eval_dsets_mode}" = "test_only" ]; then
                _score_dsets="${test_sets}"
            else
                _score_dsets="${valid_set} ${test_sets}"
            fi

            for dset in ${_score_dsets}; do
                _data="${data_feats}/${dset}"
                if "${score_obs}"; then
