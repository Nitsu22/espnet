...
# Debug logging (enh_log.sh)
log_all_ranks=1
debug_stall=1
debug_every_n=1
debug_dir=
dataloader_timeout=300
data_load_warn_sec=5
debug_iter_gap_sec=30
...
Options:
    ...
    --log_all_ranks      # Log from all ranks for debug (default="${log_all_ranks}").
    --debug_stall        # Enable stall debug logging (default="${debug_stall}").
    --debug_every_n      # Debug logging interval (default="${debug_every_n}").
    --debug_dir          # Directory for debug logs (default="${debug_dir}").
    --dataloader_timeout # DataLoader timeout seconds (default="${dataloader_timeout}").
    --data_load_warn_sec # Warn when data load exceeds seconds (default="${data_load_warn_sec}").
    --debug_iter_gap_sec # Warn when iter gap exceeds seconds (default="${debug_iter_gap_sec}").
...
if [ -z "${debug_dir}" ]; then
    debug_dir="${enh_exp}/debug"
fi

export ESPNET_LOG_ALL_RANKS="${ESPNET_LOG_ALL_RANKS:-${log_all_ranks}}"
export ESPNET_DEBUG_STALL="${ESPNET_DEBUG_STALL:-${debug_stall}}"
export ESPNET_DEBUG_EVERY_N="${ESPNET_DEBUG_EVERY_N:-${debug_every_n}}"
export ESPNET_DEBUG_DIR="${ESPNET_DEBUG_DIR:-${debug_dir}}"
export ESPNET_DATALOADER_TIMEOUT="${ESPNET_DATALOADER_TIMEOUT:-${dataloader_timeout}}"
export ESPNET_DATA_LOAD_WARN_SEC="${ESPNET_DATA_LOAD_WARN_SEC:-${data_load_warn_sec}}"
export ESPNET_DEBUG_ITER_GAP_SEC="${ESPNET_DEBUG_ITER_GAP_SEC:-${debug_iter_gap_sec}}"
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if $is_tse_task; then
            ...
            train_module=espnet2.bin.enh_train_log
        fi
