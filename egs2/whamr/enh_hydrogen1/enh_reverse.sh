...
train_reverse_set= # Name of reverse training set for creating wav_reverse.scp
valid_reverse_set= # Name of reverse validation set for creating wav_reverse.scp
...
Options:
    ...
    --train_reverse_set # Name of reverse training set for creating wav_reverse.scp (default="${train_reverse_set}")
    --valid_reverse_set # Name of reverse validation set for creating wav_reverse.scp (default="${valid_reverse_set}")
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            ...
            # Create wav_reverse.scp from train_reverse_set/valid_reverse_set's wav.scp
            if [ "${dset}" = "${train_set}" ] && [ -n "${train_reverse_set}" ]; then
                if [ -f "data/${train_reverse_set}/wav.scp" ]; then
                    # Copy train_reverse_set's wav.scp and adjust utterance IDs to match train_set's format
                    # Since utterance IDs are in the same order, replace _reverbse with _reverb
                    sed 's/_reverbse$/_reverb/' "data/${train_reverse_set}/wav.scp" > "${data_feats}${_suf}/${dset}/wav_reverse.scp"
                fi
            elif [ "${dset}" = "${valid_set}" ] && [ -n "${valid_reverse_set}" ]; then
                if [ -f "data/${valid_reverse_set}/wav.scp" ]; then
                    # Copy valid_reverse_set's wav.scp and adjust utterance IDs to match valid_set's format
                    sed 's/_reverbse$/_reverb/' "data/${valid_reverse_set}/wav.scp" > "${data_feats}${_suf}/${dset}/wav_reverse.scp"
                fi
            fi
            ...
            # Format wav_reverse.scp if it exists
            if [ -f "${data_feats}${_suf}/${dset}/wav_reverse.scp" ]; then
                # Create temporary wav_reverse.scp in data directory for format_wav_scp.sh
                if [ "${dset}" = "${train_set}" ] && [ -n "${train_reverse_set}" ]; then
                    _reverse_source="data/${train_reverse_set}/wav.scp"
                elif [ "${dset}" = "${valid_set}" ] && [ -n "${valid_reverse_set}" ]; then
                    _reverse_source="data/${valid_reverse_set}/wav.scp"
                else
                    _reverse_source=""
                fi
                if [ -n "${_reverse_source}" ] && [ -f "${_reverse_source}" ]; then
                    # Create temporary file with adjusted utterance IDs for format_wav_scp.sh
                    sed 's/_reverbse$/_reverb/' "${_reverse_source}" > "data/${dset}/wav_reverse.scp.tmp"
                    # shellcheck disable=SC2086
                    scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                        --out-filename "wav_reverse.scp" \
                        --audio-format "${audio_format}" --fs "${fs}" ${_opts} \
                        "data/${dset}/wav_reverse.scp.tmp" "${data_feats}${_suf}/${dset}" \
                        "${data_feats}${_suf}/${dset}/logs/wav_reverse" "${data_feats}${_suf}/${dset}/data/wav_reverse"
                    rm -f "data/${dset}/wav_reverse.scp.tmp"
                fi
            fi
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # NOTE: Not applying to test_sets to keep original data
            ...
            for spk in ${_spk_list} "wav" "wav_reverse";do
                if [ -f "${data_feats}/org/${dset}/${spk}.scp" ]; then
            ...
            _extra_scp_files="${_scp_list}"
            if [ -f "${data_feats}/${dset}/wav_reverse.scp" ]; then
                _extra_scp_files+="wav_reverse.scp "
            fi
            utils/fix_data_dir.sh --utt_extra_files "${_extra_scp_files} ${utt_extra_files}" "${data_feats}/${dset}"
        done
    fi
else
