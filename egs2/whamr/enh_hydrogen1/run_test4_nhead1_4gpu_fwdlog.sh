...
# Forward internal logging controls
export ESPNET_FWD_LOG_DETAIL=1
export ESPNET_FWD_LOG_SHAPES=0
...
CUDA_VISIBLE_DEVICES=0,1,2,3 ./enh.sh \
    ...
    --fs ${sample_rate} \
    --ngpu 4 \
    --python ./local/python_fwdlog_wrapper.sh \
    --ref_num 2 \
    --local_data_opts "--sample_rate ${sample_rate} --min_or_max ${min_or_max}" \
    --enh_config ./conf/tuning/train_enh_tflocoformer_test.yaml \
    --enh_exp exp/enh_train_enh_tflocoformer_test4_nhead1_4gpu_log \
