...
CUDA_VISIBLE_DEVICES=4,5,6,7 ./enh.sh \
    --train_set "${train_set}" \
    --valid_set "${valid_set}" \
    --test_sets "${test_sets}" \
    --fs ${sample_rate} \
    ...
    --enh_config ./conf/tuning/train_enh_flatflocoformer_small.yaml \
    --enh_exp exp/enh_train_enh_flatflocoformer_small_repro_hydrogen \
