...
rir_config=conf/tuning/train_rec_rir_2spk_nf_16k.yaml
rir_tag=train_rec_rir_2spk_nf_16k
rir_exp=
...
if [[ ${stage} -le 1 && ${stop_stage} -ge 1 ]]; then
    "${python}" local/prepare_two_speaker_dump.py --source data_rir_plus --output "${dumpdir}"
fi
