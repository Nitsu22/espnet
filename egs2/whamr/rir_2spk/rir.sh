  ...
  if [ "${stage}" -le 5 ] && [ "${stop_stage}" -ge 5 ]; then
    ...
    if [ "${rir_model_type}" = tflocoformer_sweep_v2_pit ]; then
      ...
      if [ "${rir_model_type}" = rec_rir_sweep_v2 ]; then
        train_data_param+="--train_data_path_and_name_and_type ${train_dir}/rir_direct.scp,rir_direct,sound "
        valid_data_param+="--valid_data_path_and_name_and_type ${valid_dir}/rir_direct.scp,rir_direct,sound "
      fi
    elif [ "${rir_model_type}" = rec_rir ] \
      || [ "${rir_model_type}" = rec_rir_lag_narrow ]; then
