...
train_config=
model_file=
wav_scp=dump/raw/tt_rir_single_clean_reverb_min_8k/wav.scp
output_dir=
device=cuda
sample_rate=8000
rir_length=8192
...
[ -z "${output_dir}" ] && \
  output_dir="${rir_exp}/inference/tt_rir_single_clean_reverb_min_8k"

if [ ! -f "${wav_scp}" ]; then
  wav_scp=data/tt_rir_single_clean_reverb_min_8k/wav.scp
fi
