...
rir_config=conf/tuning/train_rec_rir_single_clean_8192_lag_narrow.yaml
rir_tag=train_rec_rir_single_clean_8192_lag_narrow
...
if ! "${skip_train}" && [ "${stage}" -le 6 ] && [ "${stop_stage}" -ge 1 ]; then
  ...
  ./run_rec_rir_single_clean_8192_lag_narrow.sh \
    --stage "${train_stage}" \
...
if ! "${skip_inference}" && [ "${stage}" -le 7 ] && [ "${stop_stage}" -ge 7 ]; then
  ./run_infer_rec_rir_single_clean_8192.sh \
    ...
    --wav_scp "${dumpdir}/raw/${test_set}/speech_reverb.scp" \
