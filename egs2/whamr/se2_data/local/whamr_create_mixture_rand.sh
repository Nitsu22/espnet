...
if [ ! -f "${script_root}/create_wham_from_scratch_rand.py" ]; then
  echo "Error: ${script_root}/create_wham_from_scratch_rand.py not found."
  exit 1;
fi
...
tmp_script=$(mktemp "${script_root}/create_wham_from_scratch_rand_tmp.XXXXXX.py")
cp "${script_root}/create_wham_from_scratch_rand.py" "${tmp_script}"
...
echo "Log is in ${script_root}/mix_rand.log"
${train_cmd} ${script_root}/mix_rand.log python "${tmp_script}" \
