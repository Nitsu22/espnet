...
for x in tr cv tt; do
  for ddir in mix_both_rand_anechoic mix_clean_rand_anechoic mix_single_rand_anechoic noise s1_rand_reverb s2_rand_reverb mix_both_rand_reverb mix_clean_rand_reverb mix_single_rand_reverb s1_rand_anechoic s2_rand_anechoic; do
...
data=./data
rm -r ${data}/{tr,cv,tt}_mix_{both,clean,single}_rand_{anechoic,reverb}_${min_or_max}_${sample_rate} 2>/dev/null || true

for x in tr cv tt; do
  for mixtype in both clean single; do
    for cond in anechoic reverb; do
      ddir=${x}_mix_${mixtype}_rand_${cond}_${min_or_max}_${sample_rate}
      mkdir -p ${data}/${ddir}
      rootdir=${whamr_wav_dir}/wav${sample_rate}/${min_or_max}/${x}
      mixwav_dir=${rootdir}/mix_${mixtype}_rand_${cond}
      ...
      if [[ "$cond" = "reverb" ]]; then
        anechoic_mixwav_dir=${rootdir}/mix_${mixtype}_rand_anechoic
...
for mixtype in both clean single; do
  for cond in anechoic reverb; do
    tr=tr_mix_${mixtype}_rand_${cond}_${min_or_max}_${sample_rate}
    cv=cv_mix_${mixtype}_rand_${cond}_${min_or_max}_${sample_rate}
    tt=tt_mix_${mixtype}_rand_${cond}_${min_or_max}_${sample_rate}
