#!/usr/bin/env python3
"""t-SNE visualization with fixed+random RIR selection from YAML.
...
def _load_fixed_mixtures(path_cfg: Path) -> List[Tuple[str, str]]:
    cfg = _load_path_config(path_cfg)
    entries = cfg.get("fixed_mixtures", [])
    if entries in (None, ""):
        return []
    if not isinstance(entries, list):
        raise ValueError(f"{path_cfg}: 'fixed_mixtures' must be a list")

    fixed: List[Tuple[str, str]] = []
    for i, entry in enumerate(entries, start=1):
        if not isinstance(entry, dict):
            raise ValueError(f"fixed_mixtures[{i}] must be a mapping")
        uid = entry.get("uid")
        if not isinstance(uid, str) or uid.strip() == "":
            raise ValueError(f"fixed_mixtures[{i}].uid is required")
        label = entry.get("label")
        if not isinstance(label, str) or label.strip() == "":
            label = f"mixture_{i}"
        fixed.append((label, uid))
    return fixed
...
def _select_pair_uids(
    ...
    fixed_pair_uids: Optional[List[Tuple[str, str]]] = None,
    ...
    if fixed_pair_uids:
        valid_set = set(all_uids)
        for label, uid in fixed_pair_uids:
            if uid not in valid_set:
                raise ValueError(f"Fixed mixture uid not found in npz.scp: {uid}")
            if uid in fixed_uid_to_label:
                continue
            fixed_uid_to_label[uid] = label
            fixed_order.append(uid)

    if len(fixed_order) > num_pairs:
        raise ValueError(
            f"num-pairs ({num_pairs}) is smaller than fixed mixture count ({len(fixed_order)})"
        )

    remain_count = num_pairs - len(fixed_order)
    candidates = [uid for uid in all_uids if uid not in fixed_uid_to_label]

    selected_tail: List[str]
    if remain_count == 0:
        selected_tail = []
    elif mode == "head":
        selected_tail = candidates[:remain_count]
    elif mode == "random":
        rng = np.random.default_rng(pair_seed)
        indices = rng.choice(len(candidates), size=remain_count, replace=False)
        selected_tail = [candidates[int(i)] for i in indices]
    else:
        raise ValueError(f"Unsupported pair-selection mode: {mode}")

    selected = fixed_order + selected_tail
    return selected, fixed_uid_to_label, selected_tail
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="YAML/JSON file that lists fixed RIR paths (fixed_rirs) and optional fixed mixtures (fixed_mixtures).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Total number of RIRs to use. Includes fixed RIRs from YAML plus random fill.",
    )
    parser.add_argument(
        "--rir-selection-seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional PNG filename in --png-output-dir. Default: seed<pair-seed>.png",
    )
    ...
    fixed_mixtures = _load_fixed_mixtures(rir_path_config)

    pair_uids, fixed_pair_uid_to_label, random_pair_uids = _select_pair_uids(
        all_uids=npz_order,
        num_pairs=args.num_pairs,
        mode=args.pair_selection,
        pair_seed=args.pair_seed,
        fixed_pair_uids=fixed_mixtures,
    )
            ...
            meta_rows.append(
                {
                    ...
                    "pair_uid": uid,
                    "pair_source": "fixed" if uid in fixed_pair_uid_to_label else "random",
                    "pair_config_label": fixed_pair_uid_to_label.get(uid, ""),
    ...
    if args.png_name:
        ...
        seed_for_png = args.pair_seed if args.pair_seed is not None else args.rir_selection_seed
        png_name = f"seed{seed_for_png}.png"
    ...
    used = {
        ...
        "fixed_mixtures_from_config": [
            {"label": label, "uid": uid} for label, uid in fixed_mixtures
        ],
        "random_pair_uids": random_pair_uids,
