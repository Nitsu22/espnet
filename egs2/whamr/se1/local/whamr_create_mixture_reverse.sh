...
if [ $# -ne 4 ]; then
  ...
  echo " <whamr-wav> is wav generation space (reverse data will be added here)."
...
# - whamr_scripts directory and create_wham_from_scratch_reverse.py already exist
...
# Configure and run create_wham_from_scratch_reverse.py
cd ${dir}/whamr_scripts || exit 1
if [ -f ${dir}/whamr_scripts/create_wham_from_scratch_reverse.py ]; then
  echo "Creating Mixtures with reversed speaker positions (reverb_reverse)."
  sed -i -e "s#MONO = True#MONO = ${mono}#" \
         -e "s#DATA_LEN = \['max', 'min'\]#DATA_LEN = ['${min_or_max}']#" \
         -e "s#SAMPLE_RATES = \['16k', '8k'\]#SAMPLE_RATES = ['${sample_rate}']#" \
         ${dir}/whamr_scripts/create_wham_from_scratch_reverse.py
  echo "Log is in ${dir}/whamr_scripts/mix_reverse.log"
  ${train_cmd} ${dir}/whamr_scripts/mix_reverse.log python create_wham_from_scratch_reverse.py \
    --wsj0-root ${wsj_full_wav} \
    --wham-noise-root ${wham_noise} \
    --output-dir ${whamr_wav}
else
  echo "Error: create_wham_from_scratch_reverse.py not found in ${dir}/whamr_scripts/"
  exit 1
fi
