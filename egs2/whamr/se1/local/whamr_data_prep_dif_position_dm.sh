...
# Note: mix_single and anechoic are not generated by create_wham_from_scratch_dif_position_dm.py
for x in tr cv tt; do
  for ddir in noise s1_reverb s2_reverb mix_both_reverb mix_clean_reverb; do
...
rm -r ${data}/{tr,cv,tt}_mix_{both,clean}_reverb_${min_or_max}_${sample_rate} 2>/dev/null || true

for x in tr cv tt; do
  for mixtype in both clean; do
    for cond in reverb; do
      ...
      # Note: anechoic directories are not generated by create_wham_from_scratch_dif_position_dm.py
      # reverb scps
