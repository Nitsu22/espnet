...
# Note: mix_single is not generated by create_wham_from_scratch_dif_position_dm.py
for x in tr cv tt; do
  for ddir in mix_both_anechoic mix_clean_anechoic noise s1_reverb s2_reverb mix_both_reverb mix_clean_reverb s1_anechoic s2_anechoic; do
...
rm -r ${data}/{tr,cv,tt}_mix_{both,clean}_{anechoic,reverb}_${min_or_max}_${sample_rate} 2>/dev/null || true
