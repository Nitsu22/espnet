...
BOTH_DIR_RAND = BOTH_DIR + '_rand'
S1_DIR_RAND = S1_DIR + '_rand'
S2_DIR_RAND = S2_DIR + '_rand'
...
def create_wham(wsj_root, wham_noise_path, output_root):
    ...
    for splt in SPLITS:
        ...
        for wav_dir in ['wav' + sr for sr in SAMPLE_RATES]:
            for datalen_dir in DATA_LEN:
                ...
                for sfx in SUFFIXES:
                    ...
                    os.makedirs(os.path.join(output_path, CLEAN_DIR_RAND+sfx), exist_ok=True)
                    os.makedirs(os.path.join(output_path, SINGLE_DIR_RAND+sfx), exist_ok=True)
                    os.makedirs(os.path.join(output_path, BOTH_DIR_RAND+sfx), exist_ok=True)
                    os.makedirs(os.path.join(output_path, S1_DIR_RAND+sfx), exist_ok=True)
                    os.makedirs(os.path.join(output_path, S2_DIR_RAND+sfx), exist_ok=True)
        ...
        for i_utt, output_name in enumerate(utt_ids):
            rand_db_s1 = np.random.uniform(-RAND_DB, RAND_DB)
            rand_db_s2 = np.random.uniform(-RAND_DB, RAND_DB)
            rand_gain_s1 = 10 ** (rand_db_s1 / 20.0)
            rand_gain_s2 = 10 ** (rand_db_s2 / 20.0)
            ...
            for sr_i, sr_dir in enumerate(SAMPLE_RATES):
                ...
                for datalen_dir in DATA_LEN:
                    ...
                    for i_sfx, (sfx, source_pair) in enumerate(zip(SUFFIXES, sources)):
                        ...
                        s1_rand = s1_samples * rand_gain_s1
                        s2_rand = s2_samples * rand_gain_s2
                        mix_clean_rand, mix_single_rand, mix_both_rand = create_wham_mixes(
                            s1_rand, s2_rand, noise_samples
                        )

                        samps_rand = [mix_clean_rand, mix_single_rand, mix_both_rand, s1_rand, s2_rand]
                        dirs_rand = [CLEAN_DIR_RAND, SINGLE_DIR_RAND, BOTH_DIR_RAND, S1_DIR_RAND, S2_DIR_RAND]
                        for dir, samp in zip(dirs_rand, samps_rand):
                            sf.write(os.path.join(output_path, dir+sfx, output_name), samp,
                                     sr, subtype='FLOAT')
