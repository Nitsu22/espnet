...
S2_DIR = 's2'
NOISE_DIR = 'noise'
SUFFIXES = ['_anechoic_reverse', '_reverb_reverse']
...
def create_wham(wsj_root, wham_noise_path, output_root):
    ...
    for splt in SPLITS:
        ...
        for i_utt, output_name in enumerate(utt_ids):
            ...
            # Reverse speaker positions: s1 and s2 are swapped
            room = WhamRoom([utt_row['room_x'].iloc[0], utt_row['room_y'].iloc[0], utt_row['room_z'].iloc[0]],
                            ...
                            [utt_row['s2_x'].iloc[0], utt_row['s2_y'].iloc[0], utt_row['s2_z'].iloc[0]],  # s2 position for s1
                            [utt_row['s1_x'].iloc[0], utt_row['s1_y'].iloc[0], utt_row['s1_z'].iloc[0]],  # s1 position for s2
                            utt_row['T60'].iloc[0])
            room.generate_rirs()
