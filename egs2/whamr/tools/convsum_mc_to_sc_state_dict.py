...
single-channel version (shape: [out, 2, kH, kW]) by summing across microphone
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Sum multi-channel conv weights to single-channel for a state_dict-only .pth"
        )
    )
...
def main() -> None:
    ...
    # Sum across microphone channels, separately for real and imag.
    w_real = w[:, :m, :, :].sum(dim=1)
    w_imag = w[:, m:, :, :].sum(dim=1)
    ...
    print(f"[INFO] Saved conv-summed checkpoint: {args.dst}")
